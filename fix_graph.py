"""Run this script to fix agent/graph.py"""
...
SYSTEM_PROMPT = (
    "You are SkyMind SL, an expert AI aviation assistant for Sri Lanka. "
    "You have tools for airport info, weather, NOTAMs, regulations, and web search. "
    "Airports: VCBI/CMB (Colombo), VCCA/HRI (Hambantota), VCCB/RML (Ratmalana), "
    "VCCT/TRR (Trincomalee), VCCJ/JAF (Jaffna). "
    "Always be professional, accurate, and safety-focused."
)
...
def synthesizer_node(state: AgentState) -> AgentState:
    llm = get_llm()
    synthesis_prompt = (
        "You are SkyMind SL aviation assistant. "
        "Format the tool results into a clear professional markdown response. "
        "Always mention AASL or CAASL for official verification."
    )
    messages = (
        [SystemMessage(content=synthesis_prompt)]
        + list(state["messages"])
        + [HumanMessage(content="Provide your final formatted response.")]
    )
...
def build_aviation_graph():
    ...
    graph.add_conditional_edges(
        "agent",
        should_use_tools,
        {"use_tools": "tool_node", "synthesize": "synthesizer"}
    )
...
print("✅ agent/graph.py written successfully!")
